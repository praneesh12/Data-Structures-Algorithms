...
def selectionSort_same(array): 
	...
	while suffixSt != len(array):            # Num iterations len(array) --> O(len(array))
		for i in range(suffixSt,len(array)): # Num iterations len(array) --> O(len(array))
			if array[i] < array[suffixSt]:
				array[suffixSt], array[i] = array[i], array[suffixSt]
		suffixSt += 1
	return array                             # Overall complexity of the algorithms is n^2 
